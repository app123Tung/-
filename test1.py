...
for i in range(len(stock)):[span_15](start_span)[span_15](end_span)
    ...
    token = "輸入你的 bot token"  # 填入 BotFather 給的 Token[span_25](start_span)[span_25](end_span)
    chat_id = "輸入你的 telegram id"  # 填入 userinfobot 查到的 ID[span_26](start_span)[span_26](end_span)
